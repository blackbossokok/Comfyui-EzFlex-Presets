...
## 版本更新内容：
- V1.2.2:修复EzFlex-NodeSwitchGroup下拉列表bug，修复EzFlex-ParamPresetControl换预设后EzFlex-ParamPresetOutput、EzFlex-PreviewAny等无法校验输入的bug，优化EzFlex-FreeLatent自定义宽高预设位置。
- V1.2.1:修复node2.0模式提示词助手（`EzFlex-PromptHelper`）、素材加载器（`EzFlex-MediaLoader`）、素材输出（`EzFlex-MediaOut`）无法点击的bug。提示词助手（`EzFlex-PromptHelper`）和节点开关组（`NodeSwitchGroup`)新增收起展开栏。全面优化安全潜在问题，增强插件安全性。修复medialoader加载3d模型及预览图速度过慢的bug。修复promptHelper 高亮逻辑。
