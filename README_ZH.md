# Comfyui-EzFlex-Presets（V1.2.10 稳定版）
...
- 快捷加载：可快捷加载其他EzFlex节点。加载全部：模型组合加载器 / 分辨率·Latent 选择器 / 节点总控制 / 节点开关组 / 参数预设控制 / 参数输出控制 / **提示词助手** / **素材加载器** / **任意预览**；**素材输出（`MediaOut`）由素材加载器（`EzFlex-MediaLoader`）单独加载。
- 语言：可切换中英文。
- 主题：一套配色切换全部 EzFlex 面板（浅色=你原来的配色 / 藕荷 / 青苔 / 北境 / 极简冷灰 / 暖白焦糖 / 冷灰雾蓝 / 深空黑 / 莫兰迪紫灰 / 人鱼核 / 巧克力棕 / 克莱因蓝 / 云舞白 / 香蕉黄 / 勃艮第红 / 深青绿）。藕荷/青苔取自 Radix Colors（shadcn/ui 的底座），其余用各自官方色阶；另有一层原生控件兜底，输入框/系统下拉的明暗和文字色也跟主题走。配色集中在主题模块里，面板只写 CSS 变量，切换即时重绘所有已打开面板。
...
- 卡片管理：把卡片存成预设（user/EzFlex/prompts/<名称>.json；EzFlex 的配置都收在 user/EzFlex/ 下），可只存点选的几张、选中即加载、删除。
...
>
> 素材浏览器默认只能浏览 ComfyUI 自己的 `input` / `output`。要让它进别的目录，得在工具栏输入路径 → 点「保存根目录」，把该目录**显式登记为一个根**。
>
> **前提（决定这件事有没有风险）**：只有**能访问到你这个 ComfyUI 端口的人**才看得到这些根。
> - 默认监听 `127.0.0.1`（localhost）→ 外部机器连不上，**加不加根都只影响你自己**，不用紧张；
> - 用 `--listen 0.0.0.0` 开到局域网（部分整合包默认如此）、内网穿透、或反向代理暴露到公网 → 同一网络里的其他人 / 公网访问者就能列出并下载**你登记过的根目录及其子目录**里的文件。
>
> 把暴露面控制住，加根就是安全的：
> - **别把 ComfyUI 暴露到不信任的网络**（最根本的一条）。确实要暴露时，只登记**具体素材目录**：不要登记盘符、用户主目录、项目根这类宽目录 —— 插件本身也会拒绝把**整盘**（`C:\`、`D:\`、`/`）登记为根，历史遗留的整盘条目会被自动忽略并清除。
> - **登记 / 删除只允许本机**（回环 + 同源），别人和网页脚本都不能替你扩权；但**「读」不限本机** —— 上面那条前提就是它的边界。
> - **用完就删**：下拉里选中那个根 → 点「删除根目录」。内置的 `input` / `output` 删不掉（选中时删除按钮会变灰）。
> - **嵌套要一条条删**：同时登记了 `D:\素材` 和 `D:\素材\视频` 时，下拉里是两条，删一条不会连带删另一条。
...
└── web/
    ...
    ├── ezflex_theme.js      # 主题：16 套配色 → 共享 CSS 变量（--ez-*），切主题全画布即时生效
...
- 可选：提示词助手「设置·llama设置 → 进程内（llama-cpp-python）」需要 —— **不写进 requirements（避免 ComfyUI-Manager 安装时强制编译它，Windows 上常失败）**，需要时手动装：
```
pip install llama-cpp-python
```

- 可选：读对应格式的模型文件（.gguf / .onnx）才需要；缺了会跳过该文件的「模型元数据卡」，同样手动装：
```
pip install gguf onnx
```

> 也可以直接用 pyproject 的 extras 一次装齐可选依赖：`pip install -e .[llama,metadata]`
