# Comfyui-EzFlex-Presets (V1.2.4 stable)
...
- V1.2.4: Prompt Helper: tag panel dock (tile) mode with per-node restore, Windows-style cascading "move to group" menu, dock-mode auto-save, batch-aware card right-click, reference-media title showing its card. Preview generation: fixed the queue item shape (the missing 6th element killed ComfyUI's prompt worker), unet mode now uses the separate VAE, builtin workflow rebuilt per run, API-format-only import with a hint. Security: local-only guards on 25 more routes; removed 4 dead helpers.
